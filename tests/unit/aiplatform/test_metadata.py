...
_TEST_DEFAULT_TENSORBOARD = gca_tensorboard.Tensorboard(
...
def list_default_tensorboard_mock():
    with patch.object(
        ...
        list_default_tensorboard_mock.return_value = [_TEST_DEFAULT_TENSORBOARD]
...
def create_tensorboard_mock():
    with patch.object(
        ...
        create_tensorboard_mock.return_value = _TEST_DEFAULT_TENSORBOARD
...
class TestExperiments:
    ...
    def test_init_experiment_with_default_tensorboard(
        self, assign_backing_tensorboard_mock
    ):
        aiplatform.init(
            project=_TEST_PROJECT,
            location=_TEST_LOCATION,
        ...
        tb = assign_backing_tensorboard_mock.call_args.kwargs["tensorboard"]
        assert tb.name == _TEST_DEFAULT_TENSORBOARD_NAME
...
class TestTensorboard:
    ...
    def test_get_or_create_default_tb_no_existing_default(
        ...
        args = create_tensorboard_mock.call_args.kwargs
        assert len(args) == 2
        assert args["is_default"]
        assert args["display_name"].startswith("Default Tensorboard ")
        assert tensorboard.name == _TEST_DEFAULT_TENSORBOARD_NAME
